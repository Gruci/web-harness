...
종료 시에는 전체 검사와 git에 아직 추가하지 않은 소스 검사도 실행한다.
...
| 39 | 컬럼 식별자 raw 보간 | `reads_col_interp` | 바인딩 불가 자리의 f-string — 새면 미인증 SQLi |
| 40 | 쓰기 레이어 round | `writes_round` | 적재 정밀도 절삭 — 반올림은 표시 레이어 소관 |
| 41 | 배치 직접 SELECT | `batch_select` | 조회가 배치마다 흩어지는 것 |
...
이 구분이 없으면 부채가 allowlist 로 들어가 영구화되고, 영구 예외가 baseline 에 들어가
"왜 안 줄지?" 의 소음이 된다.
...
`web-ok`)은 정규식 시절 그대로다. 골든 픽스처엔 node_modules 가 없어 여섯이 `[TOOL]` 로 동결되고,
...
찍고 설치 명령을 준다. **통과로 처리하지 않는 것이 이 위임의 조건이다.** 화면 레이어는 언어팩이
아니라 `kernel/eslint.harness.mjs` 가 같은 원칙으로 위임한다 — 「게이트」의 화면 린터 문단.
